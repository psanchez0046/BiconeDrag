...
% bouData: Boussinesq number for each line of data
% ARcalcData: modulus of the amplitude ratio for each line of data
% deltaARcalcData: phase of the amplitude ratio for each line of data
...
for ite=1:length(expFilenames)
    ...
    for lin=1:filasexpFiledata        
        ...
        ARclean=1i*((pi*omegarad*R1*R1*R1*R1*eta_bulk)/(2*h))-inertia*omegarad*omegarad;
        % Intializing variables...
        lambda=1;
        Bou=[];
        ARcalc=[];
        errorAR=[];
        Bou(lambda)=((R-R1)/(1i*2*pi*omegarad*eta_bulk*R1*R1*R*R))*(ARexp-ARclean);
        timerVal = tic;
    ...
    if ispc
        dlmwrite(strcat(outputFilepath,'\',strrep(char(expFilenames(ite)),'exp','out')),results,'precision','%.14f','delimiter','\t')
    else
        dlmwrite(strcat(outputFilepath,'/',strrep(char(expFilenames(ite)),'exp','out')),results,'precision','%.14f','delimiter','\t')
    end    
