...
% interface nodes (Boussinesq-Scriven condition)
rows = [rows (Nb+2:N) (Nb+2:N) (Nb+2:N) (Nb+2:N)];
cols = [cols (Nb+2:N)-1 (Nb+2:N) (Nb+2:N)+1 (Nb+2:N)+(N+1)];
coefs = [coefs Bo*N*N*(1-(1./(2*((Nb+2:N)-1)))) ....
    -Bo*N*N*(2+(1./(((Nb+2:N)-1).*((Nb+2:N)-1))))-(1/delta_z) ....
    Bo*N*N*(1+(1./(2*((Nb+2:N)-1)))) ....
    repmat(1/delta_z, [1 length((Nb+2:N))])];
% internal nodes
[P, Q] = ndgrid(2:M, 2:N);
